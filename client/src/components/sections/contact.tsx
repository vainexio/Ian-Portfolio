          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {contactMethods.map((method, index) => (
              <a
                ...
                className={`glass-dark rounded-2xl p-8 hover:scale-105 hover:shadow-2xl hover:shadow-${method.color}/30 transition-all duration-700 group text-center ${
              ...
              >
                <div className="mb-6 group-hover:scale-125 transition-transform duration-500">
                  <i 
                    className={`${method.icon} text-5xl group-hover:animate-pulse`}
                    style={{ color: `var(--${method.color})` }}
                  ></i>
                </div>
                <h3 className={`text-xl font-bold text-${method.color} mb-2 group-hover:text-white transition-colors duration-300`}>
                  {method.label}
                </h3>
                <p className="text-gray-300 group-hover:text-white transition-colors duration-300 break-all">
                  {method.value}
                </p>
                <div className={`h-1 w-full bg-${method.color} rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500 mt-4`}></div>
              </a>
            ))}
          </div>
        </div>
